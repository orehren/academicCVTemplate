...
if (is.null(metadata) || is.null(metadata$`google-document`)) {
  # Use message() for non-critical info; Quarto will show this.
...
for (sheet_name in names(sheets_to_load)) {
  ...
  message("INFO: Reading sheet '", sheet_name, "'...")
  ...
  tryCatch({
    ...
    # If a sheet fails to load, stop the script with a clear error message.
    stop("ERROR: Failed to read sheet '", sheet_name, "'. Please check the sheet name and your permissions. Original error: ", e$message, call. = FALSE)
  })
}
